-- Pre-flight for drizzle/0022_backfill_observed_last_seen.sql. Read-only.
--
--   servel infra sql @agentkarma-db --remote KN --service db \
--     scripts/liveness-preflight.sql
--
-- 0022 decides what to NULL by EXISTS(evidence rows); its assertion judges the
...
-- A. Has transactions but tx_count = 0 — unscored, not unobserved. After 0022
...
--    means a THIRD writer feeds tx_count that 0022 does not know about — find it
