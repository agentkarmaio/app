/**
 * Generate AgentKarma's Celo keypair.
 *
 * Writes a fresh secp256k1 keypair to .keys/agentkarma-celo.json with 0600
 * permissions. Refuses to overwrite an existing keyfile. Prints ONLY the
 * public address — the private key never touches stdout or any log.
 *
 * Run:
 *   bun run scripts/generate-celo-keypair.ts
 */
...
const keyfile = resolve(keyDir, 'agentkarma-celo.json');
  ...
  JSON.stringify(
    {
      ...
      purpose: 'AgentKarma protocol identity on Celo (IdentityRegistry + ReputationRegistry writes, Self Agent ID)',
    },
    null,
    2,
  ),
...
console.log('AgentKarma Celo address:', account.address);
