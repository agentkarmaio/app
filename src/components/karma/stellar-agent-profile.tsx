/**
 * StellarAgentProfile — rendered when /agent/[wallet] resolves to a Stellar
 * wallet row that has a known ERC-8004 agentId. Reads the on-chain
 * IdentityRegistry (and, best-effort, the ReputationRegistry) on the
 * trionlabs/stellar-8004 LIVE mainnet contracts via Soroban RPC, so the page
 * reflects current chain state, not whatever the indexer last persisted.
 *
 * Stellar 8004 is MAINNET (trionlabs port, deployVersion 2026-04-11) — unlike
 * Arc there is NO testnet marker. On-chain reads are best-effort: if the
        ...
        <Card className="border-[rgb(255_255_255/0.08)] bg-[rgb(255_255_255/0.02)]">
          <CardHeader className="pb-4">
            ...
            <p className="mt-1 text-[11px] text-[#62666d]">
              Read directly from Stellar IdentityRegistry (mainnet)
            </p>
          </CardHeader>
              ...
              value={
                agent?.agentURI ? (
                  <span className="break-all text-[12px] text-muted-foreground">
                    {agent.agentURI.length > 60
                      ? `${agent.agentURI.slice(0, 60)}…`
                      : agent.agentURI}
                  </span>
                ) : (
