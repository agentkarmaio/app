  ...
  return {
    ...
    run: async (signal) => {
      if (chain === 'arc-mainnet' && path === 'transfers') {
        ...
        const { walkArcMainnetSettlement } = await import('@/indexer/arc-mainnet-settlement-walk');
        ...
        const [refresh, walk] = await Promise.allSettled([
          (async () => {
          ...
          (async () => {
            signal.throwIfAborted();
            assertIndexingLease();
            return walkArcMainnetSettlement({ signal });
          })(),
        ]);
        const settled = [ingestion, refresh, walk];
        ...
        const walkResult = (walk as PromiseFulfilledResult<Awaited<ReturnType<typeof walkArcMainnetSettlement>>>).value;
        const outcome = coverageOutcome(result.value.coverage, result.value.inserted);
        outcome.checkedCount = (outcome.checkedCount ?? 0) + scores.scored + walkResult.walletsUpdated;
