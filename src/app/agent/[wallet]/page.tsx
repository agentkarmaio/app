...
export default async function AgentProfilePage({
  ...
  if (resolved.addressClass === 'evm') {
    ...
    if (evmChain === 'arc-mainnet') {
      // resolved.chain was null on this probe path, so `deadMansSwitch` above is
      // null too; build it for the chain the probe settled on.
      const dms = (
        <Suspense fallback={null}>
          <DeadMansSwitchSection wallet={wallet} chain="arc-mainnet" />
        </Suspense>
      );
      return <ArcMainnetAgentProfile wallet={wallet} agentId={agentIdNum ?? undefined} deadMansSwitch={dms} />;
    }
